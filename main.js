...
function handleKeyPress(event,key_num=0) {
  var key_code;
  console.log(key_num);
  if(key_num!=0)
    key_code=key_num;
  ...
  if (key_code === KEY.ESC) {
    gameOver();
  } else if (moves[key_code]) {
    if(key_num==0)
      event.preventDefault();
    // Get new state
    let p = moves[key_code](board.piece);
...
function gameOver() {
  ...
  sound.pause();
  console.log(typeof(finishSound));
  finishSound.play();
...
function checkHighScore(score) {
  ...
  if (score > lowestScore) {
    const name = prompt('You got a highscore! Enter name:(Yon can write only ten letters)');
...
function click_black_button(){

}
function click_white_button(){
